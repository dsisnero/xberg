...
describe Xberg do
  describe "format_specific" do
    it "Standalone DOCX extraction using extract" do
      ...
      (__result.results.try(&.[0]).try(&.content).try(&.size) || 0).should be >=(20)
    end
    it "Standalone HWPX extraction using extract" do
      ...
      # TODO: unsupported assertion `not_error`
      (__result.results.try(&.[0]).try(&.content).try(&.size) || 0).should be >=(20)
      __result.results.try(&.[0]).try(&.content).to_s.should contain("Hello from HWPX")
    end
    it "Standalone PDF text extraction using extract" do
      ...
      # TODO: unsupported assertion `not_error`
      (__result.results.try(&.[0]).try(&.content).try(&.size) || 0).should be >=(50)
      (__result.results.try(&.[0]).try(&.content).includes?("Mallori") || __result.results.try(&.[0]).try(&.content).includes?("May")).should be_true
    end
