...
describe Xberg do
  describe "summarization" do
    it "LLM-driven abstractive summary. Skipped automatically when XBERG_LLM_API_KEY (or OPENAI_API_KEY) is not set." do
      pending! "requires XBERG_LLM_API_KEY / OPENAI_API_KEY" if ENV["XBERG_LLM_API_KEY"]?.nil? && ENV["OPENAI_API_KEY"]?.nil?
      ...
      __result.results.try(&.[0]).try(&.summary).try(&.text).to_s.should_not be_empty
(JSON.parse(__result.results.try(&.[0]).try(&.summary).try(&.strategy).try(&.to_json) || "null").as_s? || "").strip.should eq("abstractive")
    end
    it "TextRank extractive summary over a multi-paragraph plain text document. Pure-Rust, deterministic, no external services required." do
      ...
      # TODO: unsupported assertion `not_error`
      __result.results.try(&.[0]).try(&.mime_type).to_s.strip.should eq("text/plain")
      __result.results.try(&.[0]).try(&.summary).try(&.text).to_s.should_not be_empty
(JSON.parse(__result.results.try(&.[0]).try(&.summary).try(&.strategy).try(&.to_json) || "null").as_s? || "").strip.should eq("extractive")
    end
  end
end
